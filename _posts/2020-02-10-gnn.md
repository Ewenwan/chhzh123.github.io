...
[前一节]({% post_url 2020-02-10-dsl %})的图嵌入方法都是非常浅(shallow)的，通过学习得到一个矩阵/查找表，来得到结点的嵌入向量。但是这种方法有以下几点缺陷：
* 需要$O(|V|)$的参数量：没有任何参数的共享，所有结点都只有它自己的嵌入向量
...
注意到$W_k$和$B_k$都是共享参数，因此模型参数的复杂度是关于$|V|$次线性的。同时，这种方法可以扩展到未见过的结点。
...
<table><tr>
<td><img src="https://tkipf.github.io/graph-convolutional-networks/images/karate.png" border=0></td>
<td><img src="https://tkipf.github.io/graph-convolutional-networks/images/karate_emb.png" border=0></td>
</tr></table>
