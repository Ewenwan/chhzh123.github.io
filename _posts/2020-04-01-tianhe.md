...
# 方式一：交互式运行
$ yhrun -n 1 -N 1 -c 24 -p free ./matmul # 任务数，任务需要的节点数，核数，分区，执行指令
# 更多指令可通过yhrun -h查看
...
$ vim mybash.sh
#!/bin/bash
yhrun -n 1 -N 1 -c 24 -p free ./matmul
...
$ yhrun -n 1 -N 1 -p gpu_v100 ./test.py

# 批处理作业
$ vi mybash.sh
# !/bin/bash
yhrun -n 1 -N 1 -p ./test.py
$ chmod +x mybash # 为mybash.sh提供可执行权限
$ yhbatch -N 1 -p gpu_v100 ./mybash.sh
```
